...
const state = {
    ...
    newVietnameseData: [],
    inputValue: "", //lưu input người dùng nhập
...
$(".startGame")[0].addEventListener("click", () => {
    if (
        (state.inputValue < 4 || state.inputValue > 20) &&
        state.inputValue !== ""
    ) {
        $(".error")[0].innerHTML = "Min: 4; Max: 20";
...
const ListenerEvent = () => {
    // lắng nghe sự kiện người dùng chọn các ô data
    for (let i in state.newEnglishData) {
...
function remove() {
    if (state.check) {
        state.elementEnglish[
            state.keyActive1
        ].innerHTML = `<i class="fad fa-check checkTrue"></i>${state.valueEnglish}`;
        state.elementVietnamese[
            state.keyActive2
        ].innerHTML = `<i class="fad fa-check checkTrue"></i>${state.valueVietnamese}`;
        state.elementEnglish[state.keyActive1].setAttribute("disabled", "true");
        state.elementVietnamese[state.keyActive2].setAttribute(
            "disabled",
            "true"
        );
        if (state.wonCheck == state.inputValue) {
            // WIN GAME
            ...
            setTimeout(() => {
                $(
                    ".actions__won"
                )[0].innerHTML = `WON <div> <span id="minute_win">${times.minute}m</span>:<span id="second_win">${times.seconds}s</span> <div> <button id="btn__reload">Play Again<i class="fad fa-undo" style="margin-left: 8px;"></i></button>`;
            }, 2000);
